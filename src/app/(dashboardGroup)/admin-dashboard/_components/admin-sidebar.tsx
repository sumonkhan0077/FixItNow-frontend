...
import {
  ...
  Tag, BarChart3, Settings, LogOut, ShieldCheck,
} from "lucide-react";
import {
...
const navItems = [
  ...
  { title: "Technicians", url: "/admin-dashboard/technicians", icon: Wrench },
      ...
      <SidebarFooter className="p-3 space-y-2">
        ...
        <div className="flex items-center gap-3 rounded-2xl border border-border/80 bg-card p-2.5 shadow-sm">
          <Avatar className="size-9 border border-border/60 shrink-0">
            ...
            <AvatarFallback className="bg-rose-500/10 text-rose-500 font-bold text-xs uppercase">
              {user?.name ? user.name.slice(0, 2) : "AD"}
            </AvatarFallback>
          </Avatar>
          <div className="flex flex-col min-w-0">
            <p className="text-sm font-bold text-foreground truncate leading-tight">{user?.name || "Admin"}</p>
